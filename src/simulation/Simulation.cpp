...
void Simulation::BeforeSim(bool willUpdate)
{
        if (willUpdate)
        {
                ...
                // TM-mode electromagnetic wave simulation (EMWave2 port)
                if (emfOwner && emfOwner->enabled)
                {
                        FrameTime::Span span(frameTime, "EMField::update");
                        emfOwner->Update();
                }
...
Simulation::Simulation()
{
        ...
        //Create and attach the EM wave field (EMWave2 port)
        emfOwner = std::make_unique<EMField>(*this);
        emf = emfOwner.get();
