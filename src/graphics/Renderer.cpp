...
// ---------------------------------------------------------------------------
// TM-mode EM field display, ported from the renderGrid()/renderLines()/calcForce()
// of Paul Falstad's EMWave2 applet. One EM cell covers emf->cellSize pixels.
// ---------------------------------------------------------------------------
...
namespace
{
        ...
        // flood-fill grouping of force-feeling cells into rigid bodies, port of the
        // applet's calcForce()/forceSearch(); iterative to avoid stack overflows
        struct EMForceState
        {
                std::vector<uint8_t> forceMap;  // 0 = no body, otherwise body id
                std::vector<double> forceVecs;  // 2 floats per body, 256 bodies max
                int gw = 0, gh = 0;

                void Compute(const EMField &emf)
                {
                        gw = emf.gw;
                        gh = emf.gh;
                        forceMap.assign(size_t(gw) * gh, 0);
                        forceVecs.assign(256 * 2, 0.0);
                        uint8_t magno = 1;
                        for (int y = 1; y < gh - 1; y++)
                        {
                                ...
                                {
                                        int gi = x + y * gw;
                                        if (forceMap[gi] != 0 || !EMCellFeelsForce(emf.cells[gi]))
                                                continue;
                                        forceVecs[magno * 2] = 0;
                                        forceVecs[magno * 2 + 1] = 0;
                                        Search(emf, x, y, magno);
                                        if (magno == 255)
                                                return; // out of body slots
                                        magno++;
                                }
                        }
                }

                void Search(const EMField &emf, int x, int y, uint8_t magno)
                {
                        // explicit stack, one entry per pending cell
                        std::vector<int> stack;
                        stack.push_back(x + y * gw);
                        forceMap[x + y * gw] = magno;
                        while (!stack.empty())
                        {
                                int gi = stack.back();
                                stack.pop_back();
                                int cx = gi % gw, cy = gi / gw;
                                double mc = emf.GetMagX(gi - 1) - emf.GetMagX(gi + 1) +
                                            emf.GetMagY(gi - gw) - emf.GetMagY(gi + gw);
                                double bx = emf.cells[gi - gw].az - emf.cells[gi + gw].az;
                                double by = emf.cells[gi + 1].az - emf.cells[gi - 1].az;
                                forceVecs[magno * 2] += mc * bx + emf.cells[gi].jz * by;
                                forceVecs[magno * 2 + 1] += mc * by - emf.cells[gi].jz * bx;
                                const int nx[4] = { cx - 1, cx + 1, cx, cx };
                                const int ny[4] = { cy, cy, cy - 1, cy + 1 };
                                for (int k = 0; k < 4; k++)
                                {
                                        if (nx[k] < 1 || ny[k] < 1 || nx[k] >= gw - 1 || ny[k] >= gh - 1)
                                                continue;
                                        int ngi = nx[k] + ny[k] * gw;
                                        if (forceMap[ngi] == 0 && EMCellFeelsForce(emf.cells[ngi]))
                                        {
                                                forceMap[ngi] = magno;
                                                stack.push_back(ngi);
                                        }
        ...
        {
#if EMFIELD_DEBUG
                long long emdbgIters = 0;
                auto emdbg = [&emdbgIters](const char *where, double x, double y, int dir, int linemax, int gsx, int gsy) {
                        if (++emdbgIters % 200000 == 0)
                        {
                                std::cerr << "[EMDBG] renderLines " << where << " it=" << emdbgIters
                                          << " x=" << x << " y=" << y << " dir=" << dir
                                          << " linemax=" << linemax << " gs=" << gsx << "," << gsy << std::endl;
                        }
                };
#else
                long long emdbgIters = 0;
                auto emdbg = [](const char *, double, double, int, int, int, int) {};
#endif
                int lineGridSize = emf.lineDensity;
                if (lineGridSize < 10)
                        lineGridSize = 10;
                if (lineGridSize > 100)
                        lineGridSize = 100;
                int lineGridSize2 = lineGridSize * lineGridSize;
                std::vector<uint8_t> linegrid(size_t(lineGridSize2), 0);
                ...
                while (true)
                {
                        emdbg("top", x, y, dir, linemax, gridsearchx, gridsearchy);
#if EMFIELD_DEBUG
                        // debug block: hard safety net against a runaway line trace
                        if (emdbgIters > 20000000)
                        {
                                std::cerr << "[EMDBG] renderLines runaway loop broken" << std::endl;
                                break;
                        }
#endif
                        if (linemax-- == 0 || x == 0)
                        {
                                if (dir == 1)
                                {
                                        ...
                                        if (gridsearchx == lineGridSize && gridsearchy == lineGridSize)
                                                break;
                                        startx = gridsearchx / lsx;
                                        starty = gridsearchy / lsy;
                                }
                                x = startx + .48 / lsx;
                                y = starty + .48 / lsy;
                                linemax = 40; // was 100
                        ...
                        if (doArrow && linegrid[cgx + lineGridSize * cgy] == 1)
                        {
                                ...
                                {
                                        int as = 5;
                                        ren.DrawLine(Vec2{ lx2, ly2 }, Vec2{ int(dy * as - dx * as + lx2), int(-dx * as - dy * as + ly2) }, rgb);
                                        ren.DrawLine(Vec2{ lx2, ly2 }, Vec2{ int(-dy * as - dx * as + lx2), int(dx * as - dy * as + ly2) }, rgb);
                                }
                        }
                }
...
void Renderer::draw_emfield()
{
        ...
        int viewScalar = EMVIEW_OFF, viewVector = EMVIEW_OFF;
        int viewScalarCond = EMVIEW_OFF, viewVectorCond = EMVIEW_OFF;
        bool showLines = false;
        bool wantForce = false;
        switch (emViewMode)
        {
        case EMVIEW_E: case EMVIEW_A: case EMVIEW_J:
        case EMVIEW_BX: case EMVIEW_BY: case EMVIEW_HX: case EMVIEW_HY:
        case EMVIEW_EFF_CUR: case EMVIEW_MAG_CHARGE: case EMVIEW_TYPE:
        case EMVIEW_B_STRENGTH: case EMVIEW_ENERGY:
                viewScalar = viewScalarCond = emViewMode;
                break;
        case EMVIEW_B: case EMVIEW_POYNTING: case EMVIEW_H: case EMVIEW_M: case EMVIEW_CURL_E:
                viewVector = viewVectorCond = emViewMode;
                break;
        case EMVIEW_B_LINES:
                showLines = true;
                break;
        case EMVIEW_FORCE:
                viewScalar = viewScalarCond = EMVIEW_E;
                viewVector = viewVectorCond = EMVIEW_FORCE;
                wantForce = true;
                break;
        case EMVIEW_E_B:
                viewScalar = viewScalarCond = EMVIEW_E;
                viewVector = viewVectorCond = EMVIEW_B;
                emult = .3;
                break;
        case EMVIEW_E_B_J:
                viewScalar = EMVIEW_E;
                viewScalarCond = EMVIEW_J;
                viewVector = viewVectorCond = EMVIEW_B;
                emult = .3;
                break;
        case EMVIEW_E_B_LINES:
                viewScalar = viewScalarCond = EMVIEW_E;
                showLines = true;
                emult = .3;
                break;
        case EMVIEW_E_B_LINES_J:
                viewScalar = EMVIEW_E;
                viewScalarCond = EMVIEW_J;
                showLines = true;
                emult = .3;
                break;
        case EMVIEW_POYNTING_ENERGY:
                viewScalar = viewScalarCond = EMVIEW_ENERGY;
                viewVector = viewVectorCond = EMVIEW_POYNTING;
                break;
        }

        EMForceState forceState;
        if (wantForce)
        {
        ...
        for (int cy = cy0; cy < cy1; cy++)
        {
                ...
                for (int cx = cx0; cx < cx1; cx++)
                {
                        ...
                        bool edgeCell = cx < 1 || cy < 1 || cx >= gw - 1 || cy >= gh - 1;
                        if (cell.gray || cell.jz != 0 || cell.jzext != 0)
                        {
                                col_r = col_g = col_b = 64;
                                if (cell.conductivity > 0 || ((cell.jz != 0 || cell.jzext != 0) && !cell.resonant))
                                {
                                        vv = viewVectorCond;
                                        vs = viewScalarCond;
                                }
                        ...
                        double dy = 0;
                        if (vs != EMVIEW_OFF && !edgeCell)
                        {
                                switch (vs)
                                {
                                case EMVIEW_E:   dy = -cell.dazdt * emult; break;
                                ...
                                case EMVIEW_HX:
                                        ...
                                        break;
                                case EMVIEW_HY:
                                        dy = -((emf->cells[gi + 1].az - emf->cells[gi - 1].az) / cell.perm
                                              - cell.my * EM_MHMULT);
                                        break;
                                case EMVIEW_EFF_CUR:
                                        dy = emf->GetMagY(gi - 1) - emf->GetMagY(gi + 1) +
                                             emf->GetMagX(gi + gw) - emf->GetMagX(gi - gw);
                                        break;
                                case EMVIEW_MAG_CHARGE:
                                        dy = emf->cells[gi - 1].mx - emf->cells[gi + 1].mx +
                                             emf->cells[gi - gw].my - emf->cells[gi + gw].my;
                                        break;
                                ...
                                case EMVIEW_ENERGY:
                                {
                                        double n = 1 / (1 - cell.medium * EM_MEDIUM_MAX_INDEX / EM_MEDIUM_MAX);
                                        double dielec = n * n;
                                        double dx = emf->cells[gi - gw].az - emf->cells[gi + gw].az;
                                        dy = emf->cells[gi + 1].az - emf->cells[gi - 1].az;
                                        dy = .4 * ((dx * dx + dy * dy) / cell.perm +
                                                   cell.dazdt * cell.dazdt * dielec);
                                        break;
                                }
                                }
                                dy *= mult;
                                ...
                                if (vs == EMVIEW_TYPE)
                                {
                                        ...
                                        {
                                                dr = cell.medium / double(EM_MEDIUM_MAX);
                                                dg = dr * .5;
                                        }
                                        else if (cell.conductivity > 0)
                                        {
                                                dg = db = cell.conductivity;
                                                if (cell.conductivity == 1)
                                                        dr = 1;
                                        }
                                        else if (cell.jz + cell.jzext > 0)
                                        {
                                                dr = dg = (cell.jz + cell.jzext) * mult;
                                        }
                                        else if (cell.jz + cell.jzext < 0)
                                        {
                                                db = -(cell.jz + cell.jzext) * mult;
                                        }
                                        dr = std::clamp(dr, 0.0, 1.0);
                                        dg = std::clamp(dg, 0.0, 1.0);
                                        db = std::clamp(db, 0.0, 1.0);
                                        col_r = col_r + int(dr * (255 - col_r));
                                        col_g = col_g + int(dg * (255 - col_g));
                                        col_b = col_b + int(db * (255 - col_b));
                                }
                                else if (vs == EMVIEW_J || vs == EMVIEW_EFF_CUR || vs == EMVIEW_ENERGY)
                                {
                                        if (dy < 0)
                        ...
                        uint32_t col = (255 << 24) | (col_r << 16) | (col_g << 8) | col_b;
                        RGB rgb = RGB::Unpack(col);
                        if (findingElement)
                        {
                        ...
                        if (vv != EMVIEW_OFF && !edgeCell)
                        {
                                ...
                                case EMVIEW_M:
                                {
                                        double mm = 1 - 1 / cell.perm;
                                        dx = (emf->cells[gi - gw].az - emf->cells[gi + gw].az) * mm +
                                             cell.mx;
                                        dy = (emf->cells[gi + 1].az - emf->cells[gi - 1].az) * mm +
                                             cell.my;
                                        break;
                                }
                                case EMVIEW_POYNTING:
                                        dy = 5 * cell.dazdt *
                                             (emf->cells[gi - gw].az - emf->cells[gi + gw].az) / cell.perm;
                                        dx = -5 * cell.dazdt *
                                             (emf->cells[gi + 1].az - emf->cells[gi - 1].az) / cell.perm;
                                        break;
                                case EMVIEW_FORCE:
                                        dx = forceState.forceVecs[size_t(forceState.forceMap[gi]) * 2];
                                        dy = forceState.forceVecs[size_t(forceState.forceMap[gi]) * 2 + 1];
                                        break;
                                case EMVIEW_CURL_E:
                                        dx = -5 * (emf->cells[gi - gw].dazdt - emf->cells[gi + gw].dazdt);
                                        dy = -5 * (emf->cells[gi + 1].dazdt - emf->cells[gi - 1].dazdt);
                                        break;
                                }
                                double dn = std::sqrt(dx * dx + dy * dy);
                                if (dn > 0)
                                ...
                                RGB vrgb = RGB::Unpack((255 << 24) | (col_r << 16) | (col_g << 8) | col_b);
                                if (findingElement)
                                {
                                        vrgb = RGB(vrgb.Red / 10, vrgb.Green / 10, vrgb.Blue / 10);
                                }
                                // arrow through the middle of the cell, sized to the cell
                                int sw2 = std::max(cs / 2, 1);
                                int sh2 = std::max(cs / 2, 1);
                                int mx = px + cs / 2;
                                int my = py + cs / 2;
                                int x1 = mx - int(sw2 * dx);
                                int y1 = my - int(sh2 * dy);
                                int x2 = mx + int(sw2 * dx);
                                int y2 = my + int(sh2 * dy);
                                DrawLine(Vec2{ x1, y1 }, Vec2{ x2, y2 }, vrgb);
                                if (cs >= 4)
                                {
                                        int as = 3;
                                        DrawLine(Vec2{ x2, y2 }, Vec2{ int(dy * as - dx * as + x2), int(-dx * as - dy * as + y2) }, vrgb);
                                        DrawLine(Vec2{ x2, y2 }, Vec2{ int(-dy * as - dx * as + x2), int(dx * as - dy * as + y2) }, vrgb);
                                }
                        }
                }
